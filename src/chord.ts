...
export class Chord {
   ...
   */
  static buildTriadsFromScale(scale: Note[]): Note[][] {
   ...
   */
  static buildTriadAtDegree(scaleDegrees: Note[], rootDegreeIndex: number): Note[] {
  ...
  private static getScaleDegreesWithoutRepeatedRoot(scale: Note[]): Note[] {
  ...
  private static cloneNote(note: Note): Note {
    return new Note(note.name, note.octave);
  }

  private static cloneNoteWithWrapOctave(note: Note, wrapped: boolean): Note {
